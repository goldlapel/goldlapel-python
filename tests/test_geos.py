...
def fake_patterns():
    ...
    return {
        ...
        "query_patterns": {
            ...
            "georadius": f"WITH anchor AS ( SELECT ST_SetSRID(ST_MakePoint($1, $2), 4326)::geography AS geog ) SELECT member, ST_X(location::geometry) AS lon, ST_Y(location::geometry) AS lat FROM {main}, anchor WHERE ST_DWithin(location, anchor.geog, $3) ORDER BY ST_Distance(location, anchor.geog) LIMIT $4",
            "georadius_with_dist": f"WITH anchor AS ( SELECT ST_SetSRID(ST_MakePoint($1, $2), 4326)::geography AS geog ) SELECT member, ST_X(location::geometry) AS lon, ST_Y(location::geometry) AS lat, ST_Distance(location, anchor.geog) AS distance_m FROM {main}, anchor WHERE ST_DWithin(location, anchor.geog, $3) ORDER BY distance_m LIMIT $4",
